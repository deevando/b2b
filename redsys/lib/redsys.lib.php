...
/**
 *	\file			htdocs/redsys/lib/redsys.lib.php
 *	\ingroup		redsys
 *  \brief			Library for common redsys functions
 */
...
function print_paybox_redirect($PRICE, $CURRENCY, $EMAIL, $urlok, $urlko, $TAG)
{
	...
	if (!empty($conf->global->PAYBOX_IBS_SITE)) {
		$CODE = $conf->global->PAYBOX_IBS_SITE;
	}
	...
	if ($conf->global->PAYBOX_CGI_URL_V1) {
		$URLREDSYS = $conf->global->PAYBOX_CGI_URL_V1;
	}
	if ($conf->global->PAYBOX_CGI_URL_V2) {
		$URLREDSYS = $conf->global->PAYBOX_CGI_URL_V2;
	}

	if (empty($CODE)) {
		dol_print_error('', "Redsys setup param DS_MERCHANT_MERCHANTCODE not defined");
		return -1;
	}
	if (empty($URLPAYBOX)) {
		dol_print_error('', "Redsys setup param PAYBOX_CGI_URL_V1 and PAYBOX_CGI_URL_V2 undefined");
		return -1;
	}
	if (empty($IBS_SITE)) {
		dol_print_error('', "Redsys setup param PAYBOX_IBS_SITE not defined");
		return -1;
	}
	if (empty($IBS_RANG)) {
		dol_print_error('', "Redsys setup param PAYBOX_IBS_RANG not defined");
		return -1;
	}
	...
	dol_syslog("Soumission Redsys", LOG_DEBUG);
