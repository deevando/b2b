...
/**
 * \file       htdocs/redsys/admin/redsys.php
 * \ingroup    reddys
 * \brief      Page to setup redsys module
 */
...
$langs->loadLangs(array('admin', 'other', 'paybox','redsys','paypal', 'stripe'));
...
if ($action == 'setvalue' && $user->admin) {
	...
	$result = dolibarr_set_const($db, "REDSYS_CGI_URL_V1", GETPOST('REDSYS_CGI_URL_V1', 'alpha'), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
	$result = dolibarr_set_const($db, "REDSYS_CGI_URL_V2", GETPOST('REDSYS_CGI_URL_V2', 'alpha'), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
	$result = dolibarr_set_const($db, "DS_MERCHANT_MERCHANTCODE", GETPOST('DS_MERCHANT_MERCHANTCODE', 'alpha'), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
	$result = dolibarr_set_const($db, "DS_MERCHANT_TERMINAL", GETPOST('DS_MERCHANT_TERMINAL', 'alpha'), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
	...
	$result = dolibarr_set_const($db, "REDSYS_BANK_ACCOUNT_FOR_PAYMENTS", GETPOST('REDSYS_BANK_ACCOUNT_FOR_PAYMENTS', 'int'), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
	...
	if (!$result > 0) {
		...
		$result = dolibarr_set_const($db, "Ds_Signature", dol_encode(GETPOST('Ds_Signature', 'alpha')), 'chaine', 0, '', $conf->entity);
	if (!$result > 0) {
		$error++;
	}
...
$CODE = "148500556"; // Site test
if (empty($conf->global->DS_MERCHANT_MERCHANTCODE)) {
	$conf->global->DS_MERCHANT_MERCHANTCODE = $CODE;
}
$TERMINAL = "002"; // Rang test
if (empty($conf->global->DS_MERCHANT_TERMINAL)) {
	$conf->global->DS_MERCHANT_TERMINAL = $TERMINAL;
}
$CURRENCY = "978"; // Euro
if (empty($conf->global->DS_MERCHANT_CURRENCY)) {
	$conf->global->DS_MERCHANT_CURRENCY = $CURRENCY;
}
...
$head[$h][0] = DOL_URL_ROOT."/redsys/admin/redsys.php";
$head[$h][1] = $langs->trans("RedSys");
...
print $langs->trans("RedSysDesc")."<br>\n";
print '<br>';
...
print '<input size="32" type="text" name="PAYBOX_IBS_SITE" value="'.$conf->global->DS_MERCHANT_MERCHANTCODE.'">';
print '<span class="opacitymedium"><br>'.$langs->trans("Example").': 148500556 ('.$langs->trans("Test").')</span>';
print '</td></tr>';
