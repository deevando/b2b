...
/**
 *  \file       htdocs/core/modules/facture/doc/pdf_deevando.modules.php
 *  \ingroup    facture
 *  \brief      File of class to generate customers invoices from deevando model
 */
...
/**
 *	Class to manage PDF invoice template deevando
 */
class pdf_deevando extends ModelePDFFactures
{
	/**
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS)) {
			...
			if ($creditnoteamount) {
				...
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($col2x - $col1x + 20, $tab2_hl, $labeltouse, 0, 'L', 0);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
