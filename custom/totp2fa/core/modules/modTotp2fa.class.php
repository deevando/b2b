...
/**
 *      Description and activation class for the module 
 */
class modTotp2fa extends DolibarrModules
{
	...
	function __construct($db)
	{
		...
		//			'css' => '/stocktransfers/css/stocktransfers.css.php',       // Set this to relative path of css if module has its own css file
					'hooks' => array('mainloginpage','login','usercard','userlist')  // Set here all hooks context managed by module
        );
		...
		$this->config_page_url = array('config-totp.php@totp2fa');
		...
		$this->need_dolibarr_version = array(7,0,-2);	// Minimum version of Dolibarr required by module
		...
		// 'user'             to add a tab in user view
            ...
            if (!isset($conf->totp2fa) || !isset($conf->totp2fa->enabled))
            {
                    $conf->totp2fa=new stdClass();
                    $conf->totp2fa->enabled=0;
            }
		...
		// $r++;
		/*
		 $this->rights[$r][0] = 5700041; 				// Permission id (must not be already used)
		 $this->rights[$r][1] = 'Enable & disable personal 2FA TOTP';	// Permission label
		 $this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
...
*/
	...
	function init($options='')
	{
		...
		#Extrafields on users "table"
		$r = $this->db->query('SELECT * FROM '.MAIN_DB_PREFIX.'extrafields WHERE name LIKE "totp2fa_secret" AND elementtype="user"');
		if ($this->db->num_rows($r) == 0) {
			require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
			$extrafields = new ExtraFields($this->db);
			$extrafields->addExtraField('totp2fa_secret', 'totp2fa_Secret', 'varchar', 120, 255, 'user', 0, 0, '', array('options'=>array(''=>null)) 
										,1, '', '1', '', '', '', 'totp2fa@totp2fa', '1', '' );
			/*
			public function addExtraField($attrname, $label, $type, $pos, $size, $elementtype, $unique = 0, $required = 0, $default_value = '', $param = '', 
					$alwayseditable = 0, $perms = '', $list = '-1', $help = '', $computed = '', $entity = '', $langfile = '', $enabled = '1', $totalizable = 0, $printable = 0)
			*/
		}

		$result = $this->_load_tables('/totp2fa/sql/');
