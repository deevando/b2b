...
/**
 *	\file       htdocs/core/modules/fichinter/doc/pdf_deevando.modules.php
 *	\ingroup    ficheinter
 *	\brief      File of Class to build interventions documents with model Deevando
 */
...
include_once DOL_DOCUMENT_ROOT.'/includes/phpqrcode/qrlib.php';
require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';

			// $content ="deevando";
			// QRcode::png($content,$logo,QR_ECLEVEL_L,10,0);
			// $pdf->Image($logo,$posx + 2, $posy,28,28,"PNG");

/**
 *	Class to build interventions documents with model Deevando
 */
class pdf_deevando extends ModelePDFFicheinter
{
	 /**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->ficheinter->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$heightforinfotot = 180; // Height reserved to output the info and total part
				...
				$pdf->MultiCell(190, 4, "EQUIPO A REPARAR", 0, 'L', 1);
				...
				$nexY = $pdf->GetY();
				//$pdf->writeHTMLCell (190,3, $this->posxdesc-1, $tab_top-5, 'Teléfono contacto: '.$outputlangs->convToOutputCharset($object->array_options['options_tel']),0,1);
				//$pdf->MultiCell(190, 4, $outputlangs->convToOutputCharset($object->array_options['options_tel']), 0, 'L', 1);
				...
				$pdf->write(5,'Accesorios Entregados: '.$outputlangs->convToOutputCharset($object->array_options['options_accesorios']));

				$pdf->Ln();
				$pdf->write(5,'Estado Producto: '.$outputlangs->convToOutputCharset($object->array_options['options_estado']) .$outputlangs->convToOutputCharset($object->array_options['options_detest']));
				$pdf->Ln();
				$pdf->write(5,'Averia: '.$outputlangs->convToOutputCharset($object->array_options['options_averia']));
				$pdf->Ln();
				$pdf->write(5,'Teléfono Adicional: '.$outputlangs->convToOutputCharset($object->array_options['options_tel']));
				$pdf->Ln();
				$pdf->write(5,'Contraseña: '.$outputlangs->convToOutputCharset($object->array_options['pass']));
				$pdf->Ln();
				
				if ($object->array_options['options_urgente'] == true) {
					$pdf->write(5,'SERVICIO URGENTE');
				}
				...
				if ($pagenb == 1) {
				 ...
				 //$pdf->MultiCell(180,4,"HOLA ESTE ES EL PIE DE PAGINA DE MI EMPRESA",0,'C',false);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		...
		*/
		...
		if (empty($hidebottom)) {
			//$pdf->SetXY(20, 130);
			//$pdf->MultiCell(80, 5, "Autorización Cliente", 0, 'L', 1);

			$pdf->SetXY(20, 275);
			$pdf->MultiCell(80, 15, 'Autorización Cliente', 1,'L',1);

			//$pdf->SetXY(110, 130);
			//$pdf->MultiCell(80, 5, "Producto Retirado", 0, 'L', 0);

			$pdf->SetXY(110, 275);
			$pdf->MultiCell(80, 15, 'Producto Retirado', 1,'L',1);
		}
	}
...
$textref = $outputlangs->transnoentities("Ref")." : ".$outputlangs->convToOutputCharset($object->ref);
		...
		//$pdf->Cell(0, 0, $textref, 0, 1);
		$pdf->write1DBarcode($outputlangs->convToOutputCharset($object->ref), 'C128', $posx, $posy-10, '', 10, 0.2, $style, 'M');
		...
		if ($this->emetteur->logo) {
			if (is_readable($logo)) {
				...
				$pdf->Image($logo, $this->marge_gauche, $posy, 50, 0); // width=0 (auto)
			} else {
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("S.A.T.")." : ".$outputlangs->convToOutputCharset($object->ref), '', 'R');		
		...
		if ($showaddress) {
			...
			// Show movil information
			$pdf->SetFont('', 'B', $default_font_size - 1);
			$pdf->SetXY($posx + 2, $posy);
			$pdf->MultiCell($widthrecbox, 4, 'Teléfono de contacto: '. $outputlangs->convToOutputCharset($object->thirdparty->array_options['options_movil']), 0, 'L');
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_pagefoot($pdf, $outputlangs, 'FICHINTER_FREE_TEXT', '', $this->marge_basse+60, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
	}
}
